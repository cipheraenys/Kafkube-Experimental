...
export const mahasiswaService = {
  ...
  async updateMahasiswa(id, mahasiswaData) {
    try {
      const response = await apiClient.put(`/mahasiswa/${id}`, mahasiswaData);
      return response.data;
    } catch (error) {
      const errorMessage =
        error.response?.data?.error || "Gagal memperbarui mahasiswa";
      throw new Error(errorMessage);
    }
  },
  ...
  async deleteMahasiswa(id) {
    try {
      ...
      const errorMessage =
        error.response?.data?.error || "Gagal menghapus mahasiswa";
      throw new Error(errorMessage);
    }
  },
