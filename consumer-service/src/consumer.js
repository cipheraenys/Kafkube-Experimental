...
const processingHistogram = new client.Histogram({
  ...
  help: "Waktu yang dihabiskan consumer untuk memparse dan menyimpan log (detik)",
  labelNames: ["event_type"],
  buckets: [0.001, 0.005, 0.01, 0.02, 0.05, 0.1, 0.2, 0.5, 1],
});
    ...
    await consumer.run({
      eachMessage: async ({ topic, partition, message }) => {
        ...
        try {
          ...
          const processingSec = (Date.now() - tProcessingStart) / 1000.0;
          processingHistogram.labels(event.eventType || "unknown").observe(processingSec);
        } catch (parseError) {
          console.error(
...
module.exports = { app, latencyHistogram, processingHistogram };
